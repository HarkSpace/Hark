...
async function main() {
  const shouldSkip = process.env.HARK_SKIP_SKILL_LINK || process.env.CI
  const isLifecycle = Boolean(process.env.npm_lifecycle_event)
  const canPrompt = Boolean(process.stdin.isTTY && process.stdout.isTTY)
  const forcePrompt = process.env.HARK_FORCE_SKILL_LINK === '1'

  if (shouldSkip) {
    console.log('已跳过技能链接：检测到 HARK_SKIP_SKILL_LINK 或 CI。')
    return
  }
